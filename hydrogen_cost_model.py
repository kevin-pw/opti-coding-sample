...
def hydrogen_supply_cost(
    ...
    total_h2_storage_capital_cost__usd = (
        h2_storage_capacity__kg * h2_storage_capital_cost__usd_per_kg
    )

    # Determine the upper and lower storage limits based on
    # the storage capacity and the starting storage level.
    # We want the starting storage level to always be zero
    # (because we are working with cumulative sums)
    ...
    # Shift the demand supply balance by one time step to enable
    # calculating the storage level after a charging or discharging event.
    h2_demand_supply_balance_shifted__kg_per_hour = jnp.roll(
        h2_demand_supply_balance__kg_per_hour, shift=1
    )
    ...
    # We want the actual lower bound to be zero, so we move the
    # entire curve up by the lower bound.
    h2_absolute_storage_level__kg = (
        h2_relative_storage_level__kg - h2_storage_lower_bound
    )
    ...
    annual_h2_deficit__ratio = (
        -annual_h2_deficit__kg_per_year / annual_h2_demand__kg_per_year
    )
    ...
    h2_production_electricity_cost__usd_per_hour = (
        electricity_consumption__kwh_per_hour * electricity_price__usd_per_kwh
    )

    # Calculate the annual hydrogen production cost
    annual_h2_production_electricity_cost__usd_per_year = jnp.sum(
        h2_production_electricity_cost__usd_per_hour
    )
    ...
    total_costs__usd = (
        total_electrolyzer_capital_cost__usd
        + annual_h2_production_electricity_cost__usd_per_year
        * electrolyzer_lifetime__years
        + total_h2_storage_capital_cost__usd
    )
    ...
    lcoh_with_deficit_penalty__usd_per_kg = lcoh__usd_per_kg * (
        1 + annual_h2_deficit__ratio * h2_deficit_penalty_factor
    )
...
# Helper function to calculate the cumulative
# sum of an array with a lower and upper bound.
